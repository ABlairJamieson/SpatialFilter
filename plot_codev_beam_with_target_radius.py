...
def beam_centroid(xx, yy, data):
    """Return the intensity-weighted beam centroid in mm."""
    total = np.sum(data)
    if total <= 0:
        return 0.0, 0.0
    x0 = np.sum(xx * data) / total
    y0 = np.sum(yy * data) / total
...
def preprocess_argv(argv: list[str]) -> list[str]:
    ...
    for arg in argv:
        if arg.startswith("-x="):
            ...
            out.extend(["--target-radius", arg.split("=", 1)[1]])
        else:
            out.append(arg)
    return out
...
def main() -> None:
    ...
    parser.add_argument("--target-radius", "-r", type=float, default=None,
                        metavar="R_MM",
                        help="Draw a fixed radius circle centered on the beam centroid and report outside fraction")
    ...
    # Intensity-weighted beam center used by both --contour and --target-radius.
    x0, y0 = beam_centroid(xx, yy, data)
    ...
    # Optional fixed target-radius circle and outside-fraction calculation.
    if args.target_radius is not None:
        target_radius = args.target_radius
        if target_radius <= 0.0:
            raise ValueError("--target-radius must be positive, e.g. 0.5")

        frac_inside = enclosed_fraction_at_radius(xx, yy, data, target_radius, x0=x0, y0=y0)
        frac_outside = 1.0 - frac_inside if np.isfinite(frac_inside) else np.nan

        print(f"Beam centroid: x0={x0:.6e} mm, y0={y0:.6e} mm")
        print(f"Target radius: {target_radius:.6f} mm")
        print(f"Fraction inside target radius:  {frac_inside:.6f}")
        print(f"Fraction outside target radius: {frac_outside:.6f}")
        ...
        ax.text(
            0.02, 0.90 if args.contour is not None else 0.98,
            f"Rtarget = {target_radius:.3f} mm\noutside = {100.0 * frac_outside:.2f}%",
            transform=ax.transAxes,
            va="top",
            ha="left",
            color="cyan",
    ...
    print(f"Target radius: {args.target_radius}")
